...
const props = defineProps<{
  flowIds: string[]
  sourceDepartmentId: string | string[] | undefined
  targetDepartmentId: string | string[] | undefined
}>()
...
const handleMigrate = async () => {
  const flowId = props.flowIds[0]
  const targetDepartmentId = props.targetDepartmentId || '-1'
...
const handleDownload = () => {
  ...
  const oldFlowId = props.flowIds[0]
  ...
  <div class="space-y-4">
    ...
    <div v-if="!result" class="space-y-4">
      ...
      <div class="flex items-center justify-center space-x-2 mb-4">
        <input
          type="checkbox"
          :id="'demoMode-' + flowIds[0]"
          v-model="demoMode"
          class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
        >
        <label :for="'demoMode-' + flowIds[0]" class="text-sm text-gray-600">
    ...
    <div v-else class="space-y-6">
      ...
      <!-- Action Buttons -->
      <div v-if="result" class="flex justify-end mt-6 space-x-4">
        <button
